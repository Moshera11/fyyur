...
class Venue(db.Model):
    ...
    venue_id = db.relationship('show', backref='venue')
...
class Artist(db.Model):
    ...
    seeking_description = db.Column(db.String())
    artist_id = db.relationship('show', backref='artist')
...
def venues():
  ...
  data=[]
  city_st_tuple_list = db.session.query(Venue.city, Venue.state).group_by(Venue.city , Venue.state ).all() # give list of tuples of for example [('newyork','al'),('chicago','de')]
  for city_st_tuple in city_st_tuple_list:
    city_st_list=list(city_st_tuple) # convert tuple to list for example ['newyork','al']
    area={'state':city_st_list[1], 'city':city_st_list[0], 'venues':[]}
    data.append(area)
    print(data)
  ...
  for d in data:
    for venue in venues:
      ...
      print(count)
      print(d)
      if d['state']==venue_state and d['city']== venue_city:
        inf={ 'id':venue_id, 'name': name, 'num_upcoming_shows':count }#, 'num_upcoming_shows':count}
        d['venues'].append(inf)
        print('t') 
  print(data)
...
def search_venues():
  ...
  coun= len(response)
  return render_template('pages/search_venues.html', results=response, count= coun, search_term=request.form.get('search_term', ''))
...
def create_venue_submission():
  ...
  try:
    ...
    print('get genres list')
    Genres = request.form.getlist('genres')
    print(Genres)
    print('str')
    st_genres= str(Genres)
    print(st_genres)
...
def search_artists():
  ...
  coun= len(response)
  return render_template('pages/search_artists.html', results=response, count=coun, search_term=request.form.get('search_term', ''))
...
def shows():
  ...
  for X in shows_list:
    ...
    v_id= X.venue_id
    t= X.start_time
    a = Artist.query.get(X.artist_id)
    a_name= a.name
    v = Venue.query.get(X.venue_id)
    v_name= v.name
    show_inf={ 'show_start_time':t, 'artist_name':a_name, 'venue_name':v_name, 'artist_id':a_id, 'venue_id':v_id}
    data.append(show_inf)
